...
function LeavesManagementManager() {
    ...
    const defaultTab = isManager ? "students" : "staff";
...
function LeavesManagementTabs() {
    ...
    useEffect(() => {
        if (isManager && (!searchParams.get("tab") || searchParams.get("tab") === "staff")) {
            router.replace(`/admin/leaves?tab=students`, { scroll: false });
        }
    }, [isManager, searchParams, router]);
