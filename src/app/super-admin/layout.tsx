...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
    ...
    if (loading || !isSuperAdmin) {
        ...
        if (isLoggingOut) {
            return (
                <div className="min-h-screen bg-[#0A192F] flex flex-col items-center justify-center text-white space-y-4">
                    <div className="w-8 h-8 border-4 border-emerald-500/20 border-t-emerald-500 rounded-full animate-spin" />
                    <p className="text-[#8892B0] animate-pulse">Signing out securely...</p>
                </div>
            );
        }
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <header className="h-16 flex items-center justify-between px-8 bg-[#112240]/50 border-b border-white/5 backdrop-blur-md">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-sm font-medium text-[#8892B0]">Active School:</span>
                        <select 
