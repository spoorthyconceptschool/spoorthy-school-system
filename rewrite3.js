...
// 3. Add handleApproveAllGroup logic and group map logic before return (
...
const hookInsertion = `
    ...
    const handleApproveAllGroup = async (group: CoverageTask[]) => {
        if (!confirm(\`Are you sure you want to approve all suggested substitutions for \\n\${getTeacherName(group[0]?.originalTeacherId)}?\`)) return;
        ...
        try {
            ...
            for (const task of pendingTasks) {
                ...
                if (!data.success) {
                    hasError = true;
                    console.error("Task fail", task.id, data.error);
                }
            }
        } catch (e) {
            console.error(e);
        ...
        setResolving(false);
        if (hasError) {
            alert("Some periods could not be automatically approved. Please check details.");
        }
    };
...
// 4. Replace the inline mapping with our new Group Mapping component
const targetMapReplaceStart = content.indexOf(`{tasks`);
const targetMapReplaceEnd = content.indexOf(`</div>\n            )}`);

const newMapping = `{groupedTasks.filter(group => {
                        const hasMatch = group.some(t => showResolved ? t.status === "RESOLVED" : t.status === "PENDING");
                        return hasMatch;
                    }).length === 0 ? (
                                                                ...
                                                                return (
                                                                    <div key={task.id} className="flex flex-col items-start md:flex-row md:items-center justify-between p-4 bg-black/20 rounded-xl border border-white/5 gap-4">
                                                                        ...
                                                                        <div className="flex items-center justify-end w-full md:w-auto gap-3 border-t md:border-t-0 border-white/5 pt-3 md:pt-0">
                                                                            ...
                                                                            {task.status === "PENDING" && task.suggestedType && (
                                                                                <Button
                                                                                    size="sm"
                                                                                    onClick={() => handleQuickApprove(task)}
                                                                                    disabled={resolving}
                                                                                    className="h-8 px-3 text-[10px] font-black uppercase tracking-widest rounded-lg bg-accent text-black hover:bg-accent/90 shadow-lg shadow-accent/20 transition-all shrink-0"
                                                                                >
                                                                                    {resolving ? <Loader2 className="w-3 h-3 animate-spin mr-1.5" /> : <CheckCircle className="w-3 h-3 mr-1.5" />} Approve
                                                                                </Button>
                                                                            )}
...
content = content.substring(0, targetMapReplaceStart) + newMapping + content.substring(targetMapReplaceEnd);

fs.writeFileSync(path, content);
console.log('Success rewrite from base commit');
