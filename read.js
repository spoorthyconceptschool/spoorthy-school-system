const admin = require("firebase-admin");

const SERVICE_ACCOUNT = {
    projectId: "spoorthy-school-live-55917",
    clientEmail: "firebase-adminsdk-fbsvc@spoorthy-school-live-55917.iam.gserviceaccount.com",
    privateKey: `-----BEGIN PRIVATE KEY-----\\nmKQAwq0Q0dPAv5PdJCkooKsh\n-----END PRIVATE KEY-----`
};

admin.initializeApp({
    credential: admin.credential.cert(SERVICE_ACCOUNT),
    databaseURL: "https://spoorthy-school-live-55917-default-rtdb.firebaseio.com"
});

async function run() {
    process.env.NODE_NO_WARNINGS = "1";
    const doc = await admin.firestore().collection("settings").doc("branding").get();
    require("fs").writeFileSync("output.json", JSON.stringify({ fs: doc.data() }, null, 2));

    const snap = await admin.database().ref("siteContent/branding").once("value");
    const data = JSON.parse(require("fs").readFileSync("output.json"));
    data.rtdb = snap.val();
    require("fs").writeFileSync("output.json", JSON.stringify(data, null, 2));

    process.exit(0);
}

run();
